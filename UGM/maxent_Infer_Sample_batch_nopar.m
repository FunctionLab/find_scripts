function  [currentSamples,nodeBel,edgeBel,logZ] = maxent_Infer_Sample_batch_nopar(nodePot, edgePot, edgeStruct,currentSamples)
...
%      currentSamples = UGM_Sample_GibbsC_Single_Batch_OpenMP(nodePot,edgePot,edgeEnds,nStates,V,E,maxIter,int32(0),int32(currentSamples));
...
if nargout > 1
    ...
    if nargout > 3
        ...
        count=0;
        for i=1:nSamples
            if(double(currentSamples(:,i))==modes')
                count=count+1;
            end
        end
