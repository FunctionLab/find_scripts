...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
     ...
     * //     }
     * //     */
#pragma omp parallel  default(shared) private(k,i,n,s,e,z,U,u,n1,n2,Vind,pot)
{
                ...
                if(notclamp[k + n*nSamples]==1){
                    ...
                    for(Vind = V[n]-1; Vind < V[n+1]-1; Vind++)
                    {
                        ...
                        if(n == n1)
                        {
                            for(s = 0; s < nStates[n]; s++)
                            {
                                pot[s] *= edgePot[s+maxState*(y[n2+ nNodes*k] + maxState*(e + k*nEdges))];
                            }
                        ...
                        else
                        {
                            for(s = 0; s < nStates[n]; s++)
                            {
                                pot[s] *= edgePot[y[n1+ nNodes*k]+maxState*(s + maxState*(e + k*nEdges))];
                            }
                        }
