    ...
    for(i = 0; i < burnIn+maxIter; i++)
    {
        ...
        for(n = 0; n < nNodes; n++)
        {
           ...
           /* Iterate over Neighbors */
            ...
            /* Normalize  Ignored because for metropolized gibbs sampling only ratio matters*/
            /*z = 0;
            for(s = 0; s < nStates[n]; s++)
                z = z + pot[s];
            for(s = 0; s < nStates[n]; s++)
            ...
            */
            ...
            /* Ignore the first step of Metropolized Gibbs Sampler because for BINARY case the prob is always 1 */
            U = gsl_rng_uniform (r);
            u = 0;
