...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    double obs, *g, *nodeBel,*nodeBelc, *edgeBel,*edgeBelc, *Xnode, *Xedge;
        ...
        for(s = 0; s < nStates[n]; s++) {
            for(f = 0; f < nNodeFeatures; f++) {
                if(nodeMap[n + nNodes*(s + maxState*f)] > 0) {
                    ...
                    g[nodeMap[n + nNodes*(s + maxState*f)]-1] += Xnode[f]*(nodeBel[n + nNodes*s] - obs);
                }
            }
        }
    }
                ...
                for (f = 0; f < nEdgeFeatures; f++) {
                    if (edgeMap[s1 + maxState*(s2 + maxState*(e + nEdges*f))] > 0) {
                        if (Y[n1] == 0 && Y[n2] == 0)
                            ...
                            obs = edgeBelc[s1 + maxState*(Y[n2]-1 + maxState*e)];
                        else if (Y[n1]-1 == s1 && Y[n2] == 0)
                            obs = edgeBelc[Y[n1]-1 + maxState*(s2 + maxState*e)];
                        ...
                        g[edgeMap[s1 + maxState*(s2 + maxState*(e + nEdges*f))]-1] += (edgeBel[s1 + maxState*(s2 + maxState*e)] - obs);
                    }
                }
            }
        }
