function [NLL,g] = UGM_CRF_NLL_FC_Hidden(w,subXnode,subXedge,suby,nodeMap,edgeMap,edgeStruct,inferFunc,varargin)
...
for i = 1:nInstances
    ...
    [nodeBelC,edgeBelC,logZC] =  UGM_Infer_Conditional_FC(nodePot,edgePot,edgeStruct,int32(suby(i,:)),inferFunc,varargin{:});
    ...
    if edgeStruct.useMex
        ...
        % Updates in-place
        UGM_CRF_NLLC_FC_Hidden(g,int32(i),nodeBel,edgeBel,edgeEnds,nStates,nodeMap,edgeMap,subXnode(i,:),subXedge(i,:),int32(suby(i,:)),nodeBelC,edgeBelC);

    else
        NLL = NLL - (logZC+UGM_LogConfigurationPotential(suby(i,:),nodePot,edgePot,edgeEnds)) + logZ;
